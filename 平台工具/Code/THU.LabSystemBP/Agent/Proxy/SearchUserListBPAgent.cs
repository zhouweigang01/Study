...
{
    public partial class SearchUserListBPProxy : NHExt.Runtime.Model.BizAgent
    {
		...
		private string _proxyName = "THU.LabSystemBP.Agent.SearchUserListBPProxy";
 ...
 public override object DoProxy()
{
	...
	List<THU.LabSystemBE.Deploy.UserDTO> result;
	if (this.invoker.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF)
	{
		...
		try{
			result = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<List<THU.LabSystemBE.Deploy.UserDTO>>(xml);
		}
		catch(Exception ex){
	...
	{
		result= (List<THU.LabSystemBE.Deploy.UserDTO>)obj;
	}
	foreach (NHExt.Runtime.AOP.IAgentAspect aspect in aspectList)
	{
	...
	public List<THU.LabSystemBE.Deploy.UserDTO> Do()
	{
		List<THU.LabSystemBE.Deploy.UserDTO> obj = (List<THU.LabSystemBE.Deploy.UserDTO>)this.DoProxy();
		return obj;
	}

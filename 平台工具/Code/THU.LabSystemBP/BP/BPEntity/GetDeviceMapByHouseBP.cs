...
{
    public partial class GetDeviceMapByHouseBP : NHExt.Runtime.Model.BizProxy
    {
		...
		private string _proxyName = "THU.LabSystemBP.GetDeviceMapByHouseBP";
...
if(this.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF){
	this._HouseKey = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<long >(ctx.ParamList[0].ToString());
	ctx.ParamList[0] = this._HouseKey;
}
else{
	if(ctx.ParamList.Count > 0){
	this._HouseKey = (long )ctx.ParamList[0];
	}else{
		ctx.ParamList.Add(this._HouseKey);
	}
}
	}
}

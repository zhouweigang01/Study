...
{
    public partial class InsertDeviceMapBP : NHExt.Runtime.Model.BizProxy
    {
		private string _guid ="82606e37-cb5f-4f3e-a246-bfb03e227f48";
		public override string Guid {
			get{
				return this._guid;
			}
		}
		private string _proxyName = "THU.LabSystemBP.InsertDeviceMapBP";
...
internal THU.LabSystemBE.Deploy.DeviceMapDTO Do()
{
...
private THU.LabSystemBE.Deploy.DeviceMapDTO DoCommon(NHExt.Runtime.Proxy.ProxyContext ctx)
{
...
private THU.LabSystemBE.Deploy.DeviceMapDTO TypeConvert(THU.LabSystemBE.Deploy.DeviceMapDTO obj)
{
...
if(this.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF){
	this._DeviceMapDTO = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<THU.LabSystemBE.Deploy.DeviceMapDTO >(ctx.ParamList[0].ToString());
	ctx.ParamList[0] = this._DeviceMapDTO;
}
else{
	if(ctx.ParamList.Count > 0){
	this._DeviceMapDTO = (THU.LabSystemBE.Deploy.DeviceMapDTO )ctx.ParamList[0];
	}else{
		ctx.ParamList.Add(this._DeviceMapDTO);
	}
}
	}
}

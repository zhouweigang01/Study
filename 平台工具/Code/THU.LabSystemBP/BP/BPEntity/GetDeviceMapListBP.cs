...
{
    public partial class GetDeviceMapListBP : NHExt.Runtime.Model.BizProxy
    {
		...
		private string _proxyName = "THU.LabSystemBP.GetDeviceMapListBP";
...
internal List<THU.LabSystemBE.Deploy.DeviceMapDTO> Do()
{
...
private List<THU.LabSystemBE.Deploy.DeviceMapDTO> DoCommon(NHExt.Runtime.Proxy.ProxyContext ctx)
{
...
private List<THU.LabSystemBE.Deploy.DeviceMapDTO> TypeConvert(List<THU.LabSystemBE.Deploy.DeviceMapDTO> obj)
{
...
if(this.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF){
	this._DeviceKey = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<long >(ctx.ParamList[0].ToString());
	ctx.ParamList[0] = this._DeviceKey;
}
else{
	if(ctx.ParamList.Count > 0){
	this._DeviceKey = (long )ctx.ParamList[0];
	}else{

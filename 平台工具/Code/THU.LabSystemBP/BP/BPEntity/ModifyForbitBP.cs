...
{
    public partial class ModifyForbitBP : NHExt.Runtime.Model.BizProxy
    {
		...
		private string _proxyName = "THU.LabSystemBP.ModifyForbitBP";
...
internal THU.LabSystemBE.Deploy.ForbidUserDTO Do()
{
...
private THU.LabSystemBE.Deploy.ForbidUserDTO DoCommon(NHExt.Runtime.Proxy.ProxyContext ctx)
{
...
private THU.LabSystemBE.Deploy.ForbidUserDTO TypeConvert(THU.LabSystemBE.Deploy.ForbidUserDTO obj)
{
...
if(this.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF){
	this._ForbitDTO = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<THU.LabSystemBE.Deploy.ForbidUserDTO >(ctx.ParamList[0].ToString());
	ctx.ParamList[0] = this._ForbitDTO;
}
else{
	if(ctx.ParamList.Count > 0){
	this._ForbitDTO = (THU.LabSystemBE.Deploy.ForbidUserDTO )ctx.ParamList[0];
	}else{
		ctx.ParamList.Add(this._ForbitDTO);
	}
}
	}
}

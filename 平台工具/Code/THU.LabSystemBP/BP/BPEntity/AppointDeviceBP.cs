...
{
    public partial class AppointDeviceBP : NHExt.Runtime.Model.BizProxy
    {
		...
		private string _proxyName = "THU.LabSystemBP.AppointDeviceBP";
...
internal THU.LabSystemBE.Deploy.DeviceUseRecordDTO Do()
{
...
private THU.LabSystemBE.Deploy.DeviceUseRecordDTO DoCommon(NHExt.Runtime.Proxy.ProxyContext ctx)
{
...
private THU.LabSystemBE.Deploy.DeviceUseRecordDTO TypeConvert(THU.LabSystemBE.Deploy.DeviceUseRecordDTO obj)
{
...
if(this.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF){
	this._BeginTime = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<DateTime >(ctx.ParamList[1].ToString());
	ctx.ParamList[1] = this._BeginTime;
}
else{
	if(ctx.ParamList.Count > 1){
	this._BeginTime = (DateTime )ctx.ParamList[1];
	}else{
		ctx.ParamList.Add(this._BeginTime);
	}
}
if(this.CallerType == NHExt.Runtime.Session.CallerTypeEnum.WCF){
	this._EndTime = NHExt.Runtime.Serialize.XmlSerialize.DeSerialize<DateTime >(ctx.ParamList[2].ToString());
	ctx.ParamList[2] = this._EndTime;
}
else{
	if(ctx.ParamList.Count > 2){
	this._EndTime = (DateTime )ctx.ParamList[2];
	}else{

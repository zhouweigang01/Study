...
(function ($) {
    ...
    function _62(_63, _64) {
        ...
        if (_64) {
            ...
            //zhouwg增加前段事件onClick
            if(_65.options.onClick){
                _63.onclick=_65.options.onClick;
            }else if (_65.onclick) {
...
(function ($) {
    ...
    function _222(_223, _224, _225) {
        ...
        win.window({ title: _223, noheader: (_223 ? false : true), width: 350, height: "auto", modal: true, collapsible: false, minimizable: false, maximizable: false, resizable: false, onClose: function () {
            setTimeout(function () {
                win.window("destroy");
            }, 100);
        } 
...
(function ($) {
    ...
    function _2f4(_2f5, _2f6, el) {
        ...
        var _width,_height;
        var opts = $.parser.parseOptions(el, ["region"]);
        if(opts.width){
            if(opts.width<1){//zhouwg前一个if语句在多层嵌套的时候会有问题
//                if(cc.width() > 0){
//                _width=cc.width()*opts.width;
//                }else{
                _width=cc.parent().width()*opts.width;
              //  }
            }else{
                _width=opts.width;
            }
            pp.css("width",_width+"px");
        }
        if(opts.height){
            if(opts.height<1){
               // if(cc.height()>0){//zhouwg前一个if语句在多层嵌套的时候会有问题
                //_height=cc.height()*opts.height;
                //}else{
                _height=cc.parent().height()*opts.height;
               // }
            }else{
                _height=opts.height;
            }
            pp.css("height",_height+"px");
        }
         _width=(pp.length ? parseInt(pp[0].style.width) || pp.outerWidth() : "auto");
         _height=(pp.length ? parseInt(pp[0].style.height) || pp.outerHeight() : "auto");
        pp.panel($.extend({}, { 
            width: _width, 
            height: _height, 
            split: (pp.attr("split") ? pp.attr("split") == "true" : undefined), 
            doSize: false, 
            cls: ("layout-panel layout-panel-" + dir), bodyCls: "layout-body", 
            onOpen: function () {
...
(function ($) {
            ...
            if (typeof opts.toolbar == "string") {
                ...
                for (var i = 0; i < opts.toolbar.length; i++) {
                    ...
                    if (btn == "-") {
                        ...
                        $("<div class=\"easyui-toolbar-div\">"+btn+"</div>").appendTo(tb);
                        }else{
                            $("<div class=\"easyui-toolbar-div\" style=\"float:left;margin:6px 4px 6px 2px;\">"+btn+"</div>").appendTo(tb);
                        }
                    }
                    else {
    ...
    function _471(_472) {
        ...
        if(opts.view){
            opts.view.onAfterRender.call(opts.view,_472);
        }
    };
    function _483(_484) {
    ...
    function _51e(_51f, _520) {
        ...
        if (opts.view.onAfterRender) {
            opts.view.onAfterRender.call(opts.view, _51f);
        }
    ...
    function _528(_529, row) {
       ...
       // var view = $.data(_529, "datagrid").options.view;
        ...
        if (opts.view.onAfterRender) {
            opts.view.onAfterRender.call(opts.view, _529);
        }
    ...
    var _5af = { render: function (_5b0, _5b1, _5b2) {
        ...
        for (var i = 0; i < _5bf.length; i++) {
            ...
            if (col) {
                ...
                cc.push("\"");
                 //zhouwg 鼠标悬停
                if(!opts.hideQuickView){
                    if(col.formatter){
                        var title=col.formatter(_5c5, _5c2, _5c1);
                        if(title){
                        if(title.indexOf && (title.indexOf("<")>-1||title.indexOf("/>")>-1)){
        ...
        var vc = $(_5e3).datagrid('getPanel').children('div.datagrid-view');
        vc.children('div.datagrid-empty').remove();
        var hasEmptyMsg = false;
        if(this.treeNodes){
            if(this.treeNodes.length ==0){
                hasEmptyMsg =true;
            }
        }else if(!$(_5e3).datagrid('getRows').length) {
            hasEmptyMsg =true;
        }
         if (hasEmptyMsg){
            var d = $('<div class="datagrid-empty"></div>').html(opts.emptyMsg || '没有任何数据').appendTo(vc);
            d.css({
               position:'absolute',
                left:0,
                top:'50%',
    ...
    var _6c6 = $.extend({}, $.fn.datagrid.defaults.view, { render: function (_6c7, _6c8, _6c9) {
        ...
        for (var i = 0; i < _6db.length; i++) {
            ...
            if (col) {
                ...
                //zhouwg增加datagrid和treegrid鼠标悬浮提示
                cc.push("\"");
                if(!opts.hideQuickView){
                    if (col.formatter) {
                        var title=col.formatter(row[_6de], row);
                        if(title){
                            if(title.indexOf && (title.indexOf("<")>-1||title.indexOf("/>")>-1)){
...
(function ($) {
    ...
    function _790(_791) {
        ...
        $(_791).combo("setValues", vv).combo("setText", ss.join('-'));
    };
    function _793(_794, _795) {
        ...
        $(_794).combo("setValues", vv).combo("setText", ss.join('-'));
    };
